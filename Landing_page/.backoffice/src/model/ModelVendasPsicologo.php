...
class ModelVendasPsicologo {
    ...
    /* =========================
       SERVIÇOS / SESSÕES
    ========================= */
    public function getConsultas($idUtilizador) {
    ...
    public function removerVenda($idUtilizador, $idVenda) {
        ...
        $stmt = $this->conn->prepare("
            DELETE FROM venda
            WHERE id = ? AND id_prestador = ?
        ");
        $stmt->bind_param("ii", $idVenda, $codigoRH);
        $stmt->execute();
